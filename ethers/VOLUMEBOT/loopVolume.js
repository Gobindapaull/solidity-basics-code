...
const volumeBot = async () => {
    ...
    try {
        ...
        const randomBNB = (Math.random() * (process.env.MAX_BNB_BUY - process.env.MIN_BNB_BUY) + 0.001).toFixed(5);
       ...
       // 5. Delay
    var delaySeconds = Math.floor(Math.random() * (process.env.MAX_DELAY * 60 - process.env.MIN_DELAY * 60 + 1)) + 120;
